...
- **Lucide or Feather icons exclusively.** These are the "default" AI icon choice and are banned. Use `@phosphor-icons/react` (check package.json — may be pre-installed) for differentiation.
...
- **Spring physics.** Replace linear easing with spring-based motion for a natural, weighty feel on all interactive elements. `framer-motion` may be pre-installed — check package.json.
